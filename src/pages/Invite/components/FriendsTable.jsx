...
export const FriendsTable = ({ friends, loading }) => {
  ...
  return (
    <section className="bg-card-light dark:bg-card-dark rounded-xl p-6 shadow-soft border border-border-light dark:border-border-dark space-y-4">
      <h2 className="text-xl font-semibold text-gray-800 dark:text-text-dark">
          ...
          <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 border-b dark:border-gray-600">
            <tr>
              <th scope="col" className="px-4 py-3">
                {UI_TEXT.settings.friends.name}
              </th>
              <th scope="col" className="px-4 py-3 text-center">
                {UI_TEXT.settings.friends.statusLabel}
              </th>
              <th scope="col" className="px-4 py-3 text-right">
                {UI_TEXT.settings.friends.income}
              </th>
            </tr>
          </thead>
          <tbody>
            {friends.length === 0 ? (
              ...
              friends.map((friend, index) => {
                ...
                return (
                  <tr
                    key={friend.id || index}
                    className="bg-card-light dark:bg-card-dark border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      {isRewarded ? (
                        <span className="bg-accent-green/10 text-accent-green text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-accent-green/20">
                          {UI_TEXT.settings.friends.status.rewarded}
                        </span>
                      ) : (
                        <span className="bg-accent-red/10 text-accent-red text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-accent-red/20">
                          {UI_TEXT.settings.friends.status.pending}
                        </span>
                      )}
                    </td>
                    <td
                      className={`px-4 py-3 text-right font-medium ${isRewarded ? 'text-accent-green' : 'text-gray-500'}`}
                    >
                      {isRewarded ? '+1,000 Coins' : '0 Coins'}
                    </td>
                  </tr>
                )
              })
